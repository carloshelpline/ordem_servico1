...
class Marcas extends CI_Controller {
            ...
            if ($this->form_validation->run()) {
                ...
                if ($this->db->table_exists('produtos')) {
                    ...
                    if ($marca_ativa == 0 && $this->core_model->get_by_id('produtos', array('produto_marca_id' => $marca_id, 'produto_ativo' => 1))) {
                        $this->session->set_flashdata('error', 'Esta marca não pode ser desativada, pois esta sendo utilizada em Protudos');
                        redirect('marcas');
                    }
                }
    ...
    public function del($marca_id = NULL) {
        ...
        if (!$marca_id || !$this->core_model->get_by_id('marcas', array('marca_id' => $marca_id))) {
            $this->session->set_flashdata('error', 'Marca nao encontrado');
            redirect('marcas');
        } else {
            $this->core_model->delete('marcas', array('marca_id' => $marca_id));
            $this->session->set_flashdata('sucesso', 'Marca excluido com sucesso');
            redirect('marcas'); //O redirtect direciona para a pasta vendedores/index
        }
    }    
}