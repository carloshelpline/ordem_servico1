...
class Categorias extends CI_Controller {
            ...
            if ($this->form_validation->run()) {
                ...
                if ($this->db->table_exists('produtos')) {
                    ...
                    if ($categoria_ativa == 0 && $this->core_model->get_by_id('produtos', array('produto_categoria_id' => $categoria_id, 'produto_ativo' => 1))) {
                        $this->session->set_flashdata('error', 'Esta categoria não pode ser desativada, pois esta sendo utilizada em Protudos');
                        redirect('categorias');
                    }
                }
    ...
    public function del($categoria_id = NULL) {
        ...
        if (!$categoria_id || !$this->core_model->get_by_id('categorias', array('categoria_id' => $categoria_id))) {
            $this->session->set_flashdata('error', 'Categoria nao encontrado');
            redirect('categorias');
        } else {
            ...
            redirect('categorias'); //O redirtect direciona para a pasta vendedores/index
        }
    }
