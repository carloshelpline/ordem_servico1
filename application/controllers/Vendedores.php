...
//                exit();
    ...
    public function del($vendedor_id = NULL) {
        ...
        if (!$vendedor_id || !$this->core_model->get_by_id('vendedores', array('vendedor_id' => $vendedor_id))) {
            $this->session->set_flashdata('error', 'Vendedor nao encontrado');
            redirect('vendedores');
        } else {
            $this->core_model->delete('vendedores', array('vendedor_id' => $vendedor_id));
            $this->session->set_flashdata('sucesso', 'Vendedor excluido com sucesso');
            redirect('vendedores'); //O redirtect direciona para a pasta vendedores/index
        }
    }
