...
//        exit();
    ...
    public function add() {
        ...
        if ($this->form_validation->run()) {
            ...
            //Sanatização para evitar possiveis codigo malicioso no nosso html
            $data = html_escape($data);

            $this->core_model->insert('ordens_servicos', $data, TRUE);

            //Recuperar ID
            $id_ordem_servico = $this->session->userdata('last_id'); //last_id recupera o ultimo ID



            /* Tem que ser criado, pois os campos abaixo precisa ser salvas dentro de outra tabela tambem */
            $servico_id = $this->input->post('servico_id');
            $servico_quantidade = $this->input->post('servico_quantidade');
            $servico_desconto = str_replace('%', '', $this->input->post('servico_desconto')); //str_replace '%', '', retira (%) do name e substitui por ( ) nada, antes de salvar no BD

            $servico_preco = str_replace('R$', '', $this->input->post('servico_preco')); //str_replace 'R$', '', retira (R$) do name e substitui por ( ) nada, antes de salvar no BD
            $servico_item_total = str_replace('R$', '', $this->input->post('servico_item_total')); //str_replace 'R$', '', retira (R$) do name e substitui por ( ) nada, antes de salvar no BD

            $servico_preco = str_replace(',', '', $servico_preco); //str_replace 'R$', '', retira (R$) do name e substitui por ( ) nada, antes de salvar no BD
            $servico_item_total = str_replace(',', '', $servico_item_total); //str_replace 'R$', '', retira (R$) do name e substitui por ( ) nada, antes de salvar no BD
            ...
            //O for abaixo vai ser criado para percorrer a variavel $qty_servico criada acima
            // Obs. ela começa com 0 e sempre que for maior que $qty_servico $i++ adicionama +1
            ...
            $data = array(
                ...
                'scripts' => array(
                    ...
                    'vendor/calcx/jquery-calx-sample-2.2.8.min.js', //Vem segundo
                    'vendor/calcx/os.js', //Vem terceiro
    ...
    public function edit($ordem_servico_id = NULL) {
        ...
        if (!$ordem_servico_id || !$this->core_model->get_by_id('ordens_servicos', array('ordem_servico_id' => $ordem_servico_id))) {
            ...
            if ($this->form_validation->run()) {
                ...
                //unset remove o campo ordem_servico_forma_pagamento_id, tirando a obrigatoriedade de infoma-lo no post
...
//                exit();
    ...
    public function del($ordem_servico_id = NULL){
        ...
        if ($this->core_model->get_by_id('ordens_servicos', array('ordem_servico_id' => $ordem_servico_id, 'ordem_servico_status' => 0))) {
            $this->session->set_flashdata('error', 'Não é possivel excluir uma ordem de servico em aberto');
            redirect('os');
        }
    ...
    public function pdf($ordem_servico_id = NULL) {
        ...
        if (!$ordem_servico_id || !$this->core_model->get_by_id('ordens_servicos', array('ordem_servico_id' => $ordem_servico_id))) {
            ...
            $html .= '<h4 align="center">
                ...
                ' . 'Nome Fantasia: ' . $empresa->sistema_nome_fantasia . '<br/>
                ' . 'CNPJ / Insc.Estadual:  ' . $empresa->sistema_cnpj . ' &nbsp;-&nbsp; ' . $empresa->sistema_ie . '<br/>
                ' . 'Telefone(s):  ' . $empresa->sistema_telefone_movel . ' &nbsp;-&nbsp; ' . $empresa->sistema_telefone_movel . '    <br/>
                ' . 'E-mail / URL Site:  ' . $empresa->sistema_email . ' &nbsp;-&nbsp; ' . $empresa->sistema_site_url . '<br/>
                ' . 'CEP:  ' . $empresa->sistema_cep . ' &nbsp;-&nbsp; ' . $empresa->sistema_endereco . '&nbsp;-&nbsp; ' . $empresa->sistema_numero . '<br/>
...
//            exit();
            ...
            $html .= '<td style="botfrt-top: solid #ddd 1px"><strong>Valor final</strong></td>';
            $html .= '<td style="botfrt-top: solid #ddd 1px">' . 'R$&nbsp;' . $valor_final_os->os_valor_total . '</td>';
