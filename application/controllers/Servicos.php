...
//                exit();
    ...
    public function del($servico_id = NULL) {
        ...
        if (!$servico_id || !$this->core_model->get_by_id('servicos', array('servico_id' => $servico_id))) {
            $this->session->set_flashdata('error', 'Serviço nao encontrado');
            redirect('servicos');
        } else {
            $this->core_model->delete('servicos', array('servico_id' => $servico_id));
            $this->session->set_flashdata('sucesso', 'Serviço excluido com sucesso');
            redirect('servicos'); //O redirtect direciona para a pasta vendedores/index
        }
    }
