    ...
    public function add() {
        ...
        $this->form_validation->set_rules('fornecedor_razao', '', 'trim|required|min_length[4]|max_length[200]|is_unique[fornecedores.fornecedor_razao]');
            ...
            if ($this->form_validation->run()) {
                ...
                $data = html_escape($data); //Nao deixa entrar no input via post comandos indezejavel
    ...
    public function edit($fornecedor_id = NULL) {
        ...
        if (!$fornecedor_id || !$this->core_model->get_by_id('fornecedores', array('fornecedor_id' => $fornecedor_id))) {
            ...
            if ($this->form_validation->run()) {
                ...
                $fornecedor_ativa = $this->input->post('fornecedor_ativa');

                if ($this->db->table_exists('produtos')) {

                    if ($fornecedor_ativa == 0 && $this->core_model->get_by_id('produtos', array('produto_fornecedor_id' => $fornecedor_id, 'produto_ativo' => 1))) {
                        $this->session->set_flashdata('error', 'Este fornecedor não pode ser desativado, pois esta sendo utilizada em Protudos');
                        redirect('fornecedores');
                    }
                }
...
//            exit();
                ...
                /*
                 * [fornecedor_id] => 1
                  [fornecedor_data_cadastro] => 2021-06-12 01:55:57
                  [fornecedor_razao] => Carlos Helpline
                  [fornecedor_nome_fantasia] => Carlos Helpline
                  [fornecedor_cnpj] => 66.061.478/0001-30
                  [fornecedor_ie] =>
                  [fornecedor_telefone] => 83998310133
                  [fornecedor_celular] =>
                  [fornecedor_email] => carloshelpline@contato.com
                  [fornecedor_contato] =>
                  [fornecedor_cep] =>
                  [fornecedor_endereco] =>
                  [fornecedor_numero_endereco] =>
                  ...
                  [fornecedor_ativo] => 1
                  [fornecedor_obs] =>
                  [fornecedor_data_alteracao] => 2021-06-13 16:21:50
                 */
    ...
    public function check_nome_fantasia($fornecedor_nome_fantasia) {
        ...
        if ($this->core_model->get_by_id('fornecedores', array('fornecedor_nome_fantasia' => $fornecedor_nome_fantasia, 'fornecedor_id !=' => $fornecedor_id))) {
            $this->form_validation->set_message('check_nome_fantasia', 'Esta nome fantasia já existe');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    public function check_fornecedor_email($fornecedor_email) {
        ...
        if ($this->core_model->get_by_id('fornecedores', array('fornecedor_email' => $fornecedor_email, 'fornecedor_id !=' => $fornecedor_id))) {
            $this->form_validation->set_message('check_fornecedor_email', 'Esse e-mail já existe');
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    public function del($fornecedor_id = NULL){
        ...
        if(!$fornecedor_id || !$this->core_model->get_by_id('fornecedores', array('fornecedor_id' => $fornecedor_id))){
            $this->session->set_flashdata('error', 'Fornecedor nao encontrado');
            redirect('fornecedores');
        } else {
            $this->core_model->delete('fornecedores', array('fornecedor_id' => $fornecedor_id));
                $this->session->set_flashdata('sucesso', 'Fornecedor excluido com sucesso');
            redirect('fornecedores'); //O redirtect direciona para a pasta clientes/index
        }
    }
