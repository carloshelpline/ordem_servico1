    ...
    public function edit($cliente_id = NULL) {
        ...
        if (!$cliente_id || !$this->core_model->get_by_id('clientes', array('cliente_id' => $cliente_id))) {
            ...
            if ($this->form_validation->run()) {
                ...
                $cliente_ativa = $this->input->post('cliente_ativa');
                
                if ($this->db->table_exists('contas_receber')) {
                    
                    if ($cliente_ativa == 0 && $this->core_model->get_by_id('contas_receber', array('conta_receber_cliente_id' => $cliente_id, 'conta_receber_status' => 0))) {
                                                                        
                        $this->session->set_flashdata('info', 'Este cliente não pode ser desativado, pois esta sendo utilizada em <i class="fas fa-hand-holding-usd"></i>&nbsp;&nbsp;Contas a receber');
                        redirect('clientes'); //Eu disse
                    }
                }
...
//                exit();
    ...
    public function del($cliente_id = NULL){
        ...
        if(!$cliente_id || !$this->core_model->get_by_id('clientes', array('cliente_id' => $cliente_id))){
            ...
            redirect('clientes'); //O redirtect direciona para a pasta clientes/index
        }
    }
