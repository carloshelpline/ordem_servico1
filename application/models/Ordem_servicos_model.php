...
class Ordem_servicos_model extends CI_Model {
    ...
    public function get_by_id($ordem_servico_id = NULL) {
        ...
        $this->db->select([
            ...
            'CONCAT (clientes.cliente_nome, " ", clientes.cliente_sobrenome) as cliente_nome_completo', //As " " é para separar nome do sobrenome
            'formas_pagamentos.forma_pagamento_id',
            'formas_pagamentos.forma_pagamento_nome as forma_pagamento',
        ]);
        
        $this->db->where('ordem_servico_id', $ordem_servico_id); //Sempre que for Editar/Deletar temos que usar o Where
        $this->db->join('clientes', 'cliente_id = ordem_servico_cliente_id ', 'LEFT');
        $this->db->join('formas_pagamentos', 'forma_pagamento_id = ordem_servico_forma_pagamento_id ', 'LEFT');
    ...
    public function get_all_servicos_by_ordem($ordem_servicos_id = NULL) {

        if ($ordem_servicos_id) {
            ...
            $this->db->where('ordem_ts_id_ordem_servico', $ordem_servicos_id);

            return $this->db->get('ordem_tem_servicos')->result();
        }
    }

    public function delete_old_services($ordem_servicos_id = NULL) {

        if ($ordem_servicos_id) {

            $this->db->delete('ordem_tem_servicos', array('ordem_ts_id_ordem_servico' => $ordem_servicos_id));
        }
    }
